...
socket.on("Data Pack", function (temperature, ultraTime, distance, flowRate, totalFlowVortex, weight, flowRateHM, totalFlowHM , temperatureWater, valveIn, valveOut, valveSide, inverter) {
    if (true) {
        ...
        if (temperatureWater !== -1 && flowRateHM !== -1 && totalFlowHM !== -1) {
        errorflag[3] = 0;
            console.log("errorflag[3]的值" + errorflag[3]);
        }
        else if ((temperatureWater == -1 && flowRateHM == -1 && totalFlowHM == -1) && errorflag[3] < 80) {
        errorflag[3]++;
            console.log("errorflag[3]的值" + errorflag[3]);
        }
        ...
        if (isDrawingGraph) {
            ...
            chartFlow.series[0].addPoint([time, Number(flowRateHM)], true, false);
            ...
            if (isRecordingTable) {
                if (recordIntervalCounter < recordInterval) {
                    ...
                    recordingIndex++;
                    //添加表格标题行
                    console.log('表格记录是否正常:', inverter + weight + flowRateHM);
